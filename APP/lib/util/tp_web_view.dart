...
class WebViewArgument {
  ...
  const WebViewArgument({this.url, this.forceTitle});
}
...
class TPWebView extends StatelessWidget {
  ...
  TPWebView({super.key, this.titleController});
    ...
    return Scaffold(
      ...
      body: TPInAppWebView(
        onWebViewCreated: (controller) {
          ...
          print('[TPWebView] WebView 已建立，準備載入 URL: $url');
          try {
            controller.loadUrl(urlRequest: URLRequest(url: WebUri(url)));
            print('[TPWebView] URL 載入請求已發送');
          } catch (e) {
            print('[TPWebView] URL 載入失敗: $e');
            controller.loadData(data: _failedToLoadUrlData(url: url));
          }
        },
        onLoadStart: (controller, url) {
          print('[TPWebView] 開始載入頁面: $url');
        },
        onLoadStop: (controller, url) {
          print('[TPWebView] 頁面載入完成: $url');
        },
        onReceivedError: (controller, request, error) {
          print('[TPWebView] 載入錯誤: ${error.description}');
        },
        ...
        onConsoleMessage: (controller, consoleMessage) {
          // 印出 WebView 的 console 訊息到 Flutter console
          print(
    ...
    return InAppWebView(
      onWebViewCreated: (controller) async {
        await controller.addWebMessageListener(
          TPWebMessageListener.webMessageListener(),
        );
        print('[TPWebView] WebMessageListener 已註冊: flutterObject');
